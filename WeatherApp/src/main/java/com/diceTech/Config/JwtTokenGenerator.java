...
public class JwtTokenGenerator extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		System.out.println("inside method generating jwt token");
		Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
		if (null != authentication) {

			SecretKey key = Keys.hmacShaKeyFor(SecurityConstants.JWT_KEY.getBytes());

			String jwt = Jwts.builder().setIssuer("Rakshika").setSubject("JWT Token")
	...
	private String populateAuthorities(Collection<? extends GrantedAuthority> collection) {
	 ...
	 /**this make sure that this filter will execute only for first time when client call the /signIn at first time.
	 * @param request : allow request information for http servlets.
